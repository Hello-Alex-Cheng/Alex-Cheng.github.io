...
1. 异步加载 JavaScript：将 JavaScript 代码放置在 `<script>` 标签中的 `async` 或 `defer` 属性中。这样可以使 JavaScript 代码异步加载，不会阻塞 DOM 和 CSSOM 的构建，从而提高页面加载性能。
...
3. 由于js可能会操作之前的Dom节点和css样式，因此浏览器会维持html中css和js的顺序。因此，样式表会在后面的js执行前先加载执行完毕。所以`css加载会阻塞后面js的执行`。
